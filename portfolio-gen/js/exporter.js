...
<style id="pdf-export-overrides">
  ...
  function downloadHTML() {
    ...
    try {
      ...
      App.toast('HTML downloaded! âœ“');
    } catch (e) {
      console.error('HTML export error:', e);
      App.toast('Export failed â€” see console', 'error');
    }
  }
    ...
    try {
      ...
      iframe.onload = async () => {
        ...
        try {
          ...
          App.toast('PDF print dialog opened âœ“');
        } catch (err) {
          console.error('Iframe print error:', err);
          App.toast('PDF export failed â€” see console', 'error');
        } finally {
          restorePreview();
        }
      };
  ...
  function generateCV() {
    ...
    const eduHTML = education.map(e => `
      <div class="cv-edu">
        ...
        <div class="cv-edu-deg">${esc(e.degree)}${e.field ? ` Â· ${esc(e.field)}` : ''}</div>
        <div class="cv-edu-dates">${esc(e.from || '')}${e.to ? ` â€“ ${e.current ? 'Present' : esc(e.to)}` : ''}</div>
      </div>`).join('');
    ...
    <main class="cv-page">
      ...
      <div class="cv-footer">Generated by PortfolioForge Â· ${new Date().getFullYear()}</div>
    </main>`;

    const html = Renderer.buildDocument(body, css, `${personal.name || 'My'} â€” CV`);
    const name = (personal.name || 'cv').toLowerCase().replace(/\s+/g, '-');
    downloadFile(`${name}-resume.html`, html, 'text/html');
    App.toast('CV downloaded! Open in browser and Ctrl+P to save as PDF âœ“');
  }
